...
class Embeds:
    ...
    def coin_game(number, *args, **kwargs):
        if number == 0:
            tmp = Embed(
                title="Подбрасываю монетку!", colour=Color.from_rgb(255, 215, 0)
            )
            tmp.set_image(
                url="https://cdn.discordapp.com/attachments/1227605957086019615/1228682002308399164/-50.gif?ex=662cee49&is=661a7949&hm=d69f16298361988d3840daf2f7e9349edcc33f6bd1d4d22b9eddfd9b9ce8833c&"
            )
            return tmp
        if number == 1:
            return Embed(
                title="Как думаешь, что выпало?\n"
                "1. Решка\n"
                "2. Орел\n"
    ...
    def cities_game(number, *args, **kwargs):
        ...
        if number == 0:
            tmp = Embed(
                title="Давай играть в города России!", colour=Color.from_str(game_color)
            )
            tmp.set_image(
                url="https://media1.tenor.com/m/HWRcQJwQSjUAAAAC/cn-tower-toronto.gif"
            )
            return tmp
        if number == 1:
            tmp = Embed(
                title="Правила игры такие:",
        ...
        if number == 2:
            tmp = Embed(
                title=args[0],
                description=f"Этот город имеет население {args[1]['population']} чел.\n"
        ...
        if number == 3:
            tmp = Embed(
                title="Ой, кажется я не могу найти подходяший город! Похоже ты победил",
        ...
        if number == 4:
            tmp = Embed(
                title="Такого города в России нет!",
                description=f"**Осталось попыток: {3 - args[0]}**",
                colour=Color.from_str(game_color),
            )
            return tmp
        if number == 5:
            tmp = Embed(
                title=f"Название города начинается не на ту букву!\nНужна буква {args[0]}",
